              ...
              {method === "crypto" && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs mt-1">Min deposit: {selectedNetwork.minDeposit} USDT</p>
                  </div>
                  ...
                  {/* Deposit Address - only show when amount entered */}
                  {amount && Number(amount) > 0 && (
                    <div className="p-4 bg-[#1a1f26] rounded-xl border border-gray-700 space-y-3">
                      <p className="text-gray-300 text-sm text-center">
                        Send <span className="text-cyan-400 font-bold">{Number(amount).toFixed(2)} {selectedNetwork.token}</span> to:
                      </p>
                      ...
                      <p className="text-gray-500 text-xs text-center">
                        Arrival: {selectedNetwork.estimatedTime}
                      </p>
                    </div>
                  )}
                  ...
                  <button
                    onClick={() => {
                      if (!amount || Number(amount) < selectedNetwork.minDeposit) {
                        toast.error(`Minimum deposit is ${selectedNetwork.minDeposit} USDT`);
                        return;
                      }
                      toast.success("Once your transaction is confirmed, your balance will update automatically.");
                      resetAndClose();
                    }}
                    disabled={!amount || Number(amount) < selectedNetwork.minDeposit}
                    style={{
                      backgroundColor: amount && Number(amount) >= selectedNetwork.minDeposit ? '#06f6ff' : '#334155',
                      color: amount && Number(amount) >= selectedNetwork.minDeposit ? 'black' : 'white',
                      padding: '12px',
                      borderRadius: '12px',
                      width: '100%',
                      fontWeight: 600,
                      cursor: amount && Number(amount) >= selectedNetwork.minDeposit ? 'pointer' : 'not-allowed'
                    }}
                  >
                    {!amount || Number(amount) === 0 ? 'Enter amount' :
                     Number(amount) < selectedNetwork.minDeposit ? `Min ${selectedNetwork.minDeposit} USDT` :
                     'Done'}
                  </button>
                </div>
              )}
