...
type PaymentMethod = "crypto" | "mobile" | "bank" | null;
...
export default function WithdrawWallet({ isOpen, onClose }: WithdrawWalletProps) {
  ...
  const amountInLocal = amount ? Number(amount) * currency.rate : 0;
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-bold text-sm">Withdraw Funds</h2>
              <p className="text-gray-500 text-xs">Send funds to your wallet</p>
            </div>
          </div>
          <button
            type="button"
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">Direct to bank account • Fee: 2.9%</p>
                    </div>
                  </div>
                  <ChevronRight className="w-5 h-5 text-gray-500" />
                </div>
              ...
              <div className="p-3 bg-[#1a1f26] border border-gray-800 rounded-xl mt-4">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-gray-400 text-xs">All withdrawals are secured with encryption. Funds are sent from your USDT balance.</p>
                </div>
              </div>
            </div>
          )}
              ...
              {isAmountValid && (
                <div style={{ padding: '16px', backgroundColor: '#1a1f26', borderRadius: '12px' }}>
                  <div className="flex justify-between mb-3">
                    ...
                    <span style={{ color: '#fff' }}>USDT {Number(amount).toLocaleString()}</span>
                  </div>
                  {currency.code !== "USD" && (
                    <div className="flex justify-between mb-3">
                      <span style={{ color: '#9ca3af', fontSize: '14px' }}>Equivalent ({currency.code})</span>
                      <span style={{ color: '#fff' }}>{currency.symbol}{amountInLocal.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between mb-3">
                    <span style={{ color: '#9ca3af', fontSize: '14px' }}>Fee ({method === "mobile" ? "1.2%" : "2.9%"})</span>
                    <span style={{ color: '#9ca3af' }}>-USDT {(Number(amount) * (method === "mobile" ? 0.012 : 0.029)).toFixed(2)}</span>
                  </div>
                  <div style={{ borderTop: '1px solid #374151', paddingTop: '12px', marginTop: '4px' }} className="flex justify-between">
                    ...
                    <span style={{ color: '#fff', fontWeight: '600', fontSize: '16px' }}>{currency.symbol}{(amountInLocal * (1 - (method === "mobile" ? 0.012 : 0.029))).toFixed(2)}</span>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  {amount && Number(amount) > 0 && (
                    <div className="p-4 bg-[#1a1f26] rounded-xl border border-gray-700 space-y-3">
                      <p className="text-gray-300 text-sm text-center">
                        Withdraw <span className="text-cyan-400 font-bold">USDT {Number(amount).toFixed(2)}</span> to:
                      </p>
                      <div className="flex items-center gap-2 p-2 bg-gray-800 rounded-lg">
                        <input
                          ...
                          className="text-cyan-400 text-xs flex-1 bg-transparent outline-none"
                        />
                      ...
                      <p className="text-gray-500 text-xs text-center">
                        Min: {selectedNetwork.minWithdraw} USDT • Fee: {selectedNetwork.fee} USDT • {selectedNetwork.estimatedTime}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex items-start gap-2 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                    ...
                    <p className="text-yellow-500/80 text-xs">
                      Only withdraw <span className="font-bold">{selectedNetwork.token}</span> on <span className="font-bold">{selectedNetwork.name}</span>. Wrong network = lost funds.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    disabled={!amount || Number(amount) < selectedNetwork.minWithdraw || !walletAddress}
                    style={{
                      backgroundColor: amount && Number(amount) >= selectedNetwork.minWithdraw && walletAddress ? '#06f6ff' : '#334155',
                      color: amount && Number(amount) >= selectedNetwork.minWithdraw && walletAddress ? 'black' : 'white',
                      padding: '12px',
                      borderRadius: '12px',
                      width: '100%',
                      fontWeight: 600,
                      cursor: amount && Number(amount) >= selectedNetwork.minWithdraw && walletAddress ? 'pointer' : 'not-allowed'
                    }}
                  >
                    Withdraw Funds
                  </button>
              ...
              {method === "bank" && (
                <>
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: bankName && accountNumber && accountName ? '#06f6ff' : '#334155',
                      color: bankName && accountNumber && accountName ? 'black' : 'white',
                      padding: '12px',
                      borderRadius: '12px',
                      width: '100%',
                      fontWeight: 600,
                      cursor: bankName && accountNumber && accountName ? 'pointer' : 'not-allowed',
                    }}
                  >
                    Withdraw {currency.symbol}{amountInLocal.toLocaleString()}
                  </button>
                </>
              )}
            </div>
